...
<template>
    <div class="w-full bg-white grid grid-cols-3 gap-6 p-4 rounded-md cursor-pointer" :class="{'grid !grid-cols-1 !p-4 border border-gray-200': column}" @click="$router.push({path: '/post/' + postCard?.slug})">
        ...
        <div class="col-span-2 flex flex-col gap-4 justify-between" :class="{'!gap-2': small}">
            ...
            <h3 class="text-4xl text-gray-900 !mb-0" :class="{'!text-lg !leading-6 !mb-1': small}">{{postCard?.title}}</h3>
            <div class="w-fit flex flex-row text-md text-[#777777] divide-x-2 divide-gray-200" :class="{'hidden': small}">
                <span class="px-2">{{ postCard?.author?.name }}</span>
                <span class="px-2">02 december 2022</span>
                <span class="px-2" :class="{'hidden': column}">3 min. to read</span>
            </div>
            <p class="text-lg text-[#555555]" :class="{'hidden': small}">{{ postCard?.description }}</p>
        </div>
    </div>
</template>